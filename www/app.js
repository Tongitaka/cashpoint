/* Logic & Regex Parser for Cashpoint Automation */
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        await initDB();
        await checkAutoReset(); // Auto-reset ny 1er-n'ny volana
        await refreshUI();
    } catch (err) {
...
// Lojika Auto-Reset isaky ny 1er amin'ny volana vaovao
...
function launchUSSDCall() {
    ...
    if (window.plugins && window.plugins.callNumber) {
        window.plugins.callNumber.callNumber(() => console.log("USSD Appel mandeha..."), (err) => alert("Tsy nahalasa ny appel: " + err), formattedCode, true);
    } else {
...
function lancerAchatCreditMvola() {
    ...
    if (window.plugins && window.plugins.callNumber) {
        window.plugins.callNumber.callNumber(() => console.log("USSD Achat Crédit mandeha..."), (err) => alert("Tsy nahalasa ny appel: " + err), ussdCode, true);
...
// Ny asa manisa (Kajy Bonus, isan'ny mouvement & UI)
async function refreshUI() {
    try {
        ...
        startOfWeek.setDate(now.getDate() - now.getDay()); // Alahady farany teo
        ...
        if (transactions.length === 0) {
            tbodyDashboard.innerHTML = `<tr><td colspan="5" style="text-align:center;">Tsy mbola misy tranzaksion...</td></tr>`;
            tbodyJournal.innerHTML = `<tr><td colspan="8" style="text-align:center;">Tsy mbola misy tranzaksion...</td></tr>`;
            document.getElementById('footer-solde-farany').innerText = "0 Ar";
        } else {
            // Maka ny solde tamin'ny SMS farany
            soldeFarany = transactions[0].solde;

            transactions.forEach((tx, idx) => {
                const txDate = new Date(tx.timestamp);

                // Fanisana isan'ny mouvement
        ...
        // Stats Updates (Mampiditra ny an'ny Compteur sy Solde)
...
// Export CSV Function (Azo sokafana Excel)
...
async function famafanaArovanaAminKaody() {
    const kaodyNampidirina = prompt("Ampiarovana ny data!\nAmpidiro ny kaody miafina (1234) hamafana ny Bokin'ny Vola rehetra:");
