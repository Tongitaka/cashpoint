...
function initSMSListener() {
    // Hamarinina aloha raha efa vonona ny Plugin
    if (window.SMSReceive) {
        // Mangataka permission amin'ny mpampiasa amin'ny finday
        window.SMSReceive.hasPermission((hasPermission) => {
            if (!hasPermission) {
                window.SMSReceive.requestPermission(() => {
                    startSMSWatch();
                }, (err) => {
                    alert("Tsy afaka mamaky SMS ny app raha tsy omenao alalana (Permission)!");
                });
            } else {
                startSMSWatch();
            }
        }, (err) => console.error("Erreur check permission:", err));
    } else {
        console.log("Mbola tsy vonona ny SMS Plugin na amin'ny Browser no ihodinana.");
    }
}

function startSMSWatch() {
    window.SMSReceive.startWatch(() => {
        console.log("Mihaino SMS miditra ny app...");
        document.addEventListener('onSMSArrive', (e) => {
            // Rehefa misy SMS tonga dia amboarina hatrany
